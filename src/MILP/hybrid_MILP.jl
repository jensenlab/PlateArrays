...
function hybrid_MILP(P::Int,N::Int,plate::BitMatrix;MILP_timelimit=100,MILP_output=true,kwargs...)
    ...
    R,C=size(plate) # get the number of rows and columns in a plate
    ...
    for row in 1:R
        for col in 1:C
            ...
            @constraint(model, x[row,col] <= plate[row,col]) # controls can only be placed in active wells
            @constraint(model,y[row,col] <= plate[row,col]) # controls can only be placed in active wells

        end
    end
    

    epr,epc,enr,enc=expected_LHS(P,N,plate) # given the plate shape, find the expected number of controls per row and column in a uniformly distributed array of controls. 
    ...
    out=PlateArray(P,N,plate,pos,neg)
